...
@app.post("/api/stands")
def create_stand():
    """Cadastro de stand — feito pelo professor/organizador (exige senha)."""
    data   = request.get_json(silent=True) or {}
    name   = " ".join(str(data.get("name") or "").split())
    course = str(data.get("course") or "")

    if not store.verify_admin_password(str(data.get("password") or "")):
    ...
    integrantes = []
    for nome_integrante in (data.get("integrantes") or []):
        nome_integrante = " ".join(str(nome_integrante or "").split())[:100]
        if nome_integrante and nome_integrante not in integrantes:
            integrantes.append(nome_integrante)
    if not integrantes:
        return jsonify({"error": "Informe ao menos um integrante do grupo."}), 400

    tipo_projeto = " ".join(str(data.get("tipo_projeto") or "").split())[:120]
    link_projeto = str(data.get("link_projeto") or "").strip()[:300]

    githubs = []
    if course in store.GITHUB_COURSES:
        for link in (data.get("githubs") or []):
            link = str(link or "").strip()[:200]
            if link:
                githubs.append(link)

    tipos_arquivo = []
    for tipo in (data.get("tipos_arquivo") or []):
        tipo = str(tipo or "")
        if tipo in store.VALID_FILE_TYPES and tipo not in tipos_arquivo:
            tipos_arquivo.append(tipo)

    try:
        stand, access_code = store.create_stand(
            name, course,
...
def save_profile():
    ...
    for c in (data.get("cursos") or []):
        c = str(c or "").strip()
        if c in store.ALUNO_COURSES and c not in cursos:
            cursos.append(c)
    ...
    if profile_type == "aluno":
        ...
        # (caminho rápido), digitando o nome de um grupo ainda sem stand ou
        # avisando que não tem stand (ex.: cursos que não expõem projeto).
        sem_stand = data.get("no_stand") is True
        if not own_stand_ids and not sem_stand and not 2 <= len(group_name) <= 100:
            return jsonify({
                "error": "Marque o stand do seu grupo, digite o nome de um grupo ainda não cadastrado "
                         "ou marque \"Não tenho stand na feira\"."
            }), 400
    else:
        cursos        = []
        own_stand_ids = []
