...
# A 1ª Feira Tech expõe stands só destes 3 cursos. Os demais cursos da
# Vocação (ver ALUNO_COURSES) só servem para identificar quem visita —
# quando for a feira desses cursos, é só ampliar este conjunto.
VALID_COURSES = {"webdesign", "programacao", "audiovisual"}

# Cursos que o aluno pode marcar no credenciamento: os três que expõem
# stand nesta feira e os demais cursos da Vocação (só para saber quem
# visitou — não geram stand nesta edição).
ALUNO_COURSES = VALID_COURSES | {"ppt", "informatica", "excel", "rh", "administrativo"}

# Cursos cujos alunos colocam link de GitHub individual no stand.
GITHUB_COURSES = {"programacao", "webdesign"}

# Tipo de arquivo/material entregue pelo grupo (Produção Audiovisual usa
# bastante fotos/vídeos/podcast; Programação e Web Design geralmente usam
# link do projeto e executável).
VALID_FILE_TYPES = {"executavel", "fotos", "videos", "podcast", "documento", "outro"}

FILE_TYPE_LABELS = {
    "executavel": "Arquivo executável",
    "fotos":      "Fotos",
    "videos":     "Vídeos",
    "podcast":    "Podcast",
    "documento":  "Documento / Slides",
    "outro":      "Outro",
}
...
CREATE TABLE IF NOT EXISTS stands (
    ...
    -- Cadastrado pelo professor: integrantes do grupo, tipo de projeto,
    -- link (site/vídeo/etc.), GitHub de cada aluno (só Programação/Web
    -- Design) e o(s) tipo(s) de arquivo entregue.
    integrantes    TEXT NOT NULL DEFAULT '[]',
    tipo_projeto   TEXT NOT NULL DEFAULT '',
    link_projeto   TEXT NOT NULL DEFAULT '',
...
def _migrate_stands_columns(conn: sqlite3.Connection) -> None:
    """Bancos criados antes destes campos ganham as colunas novas aqui."""
    existentes = {row[1] for row in conn.execute("PRAGMA table_info(stands)")}
    novas = {
        "integrantes":   "TEXT NOT NULL DEFAULT '[]'",
        "tipo_projeto":  "TEXT NOT NULL DEFAULT ''",
        "link_projeto":  "TEXT NOT NULL DEFAULT ''",
    ...
    for coluna, definicao in novas.items():
        if coluna not in existentes:
            conn.execute(f"ALTER TABLE stands ADD COLUMN {coluna} {definicao}")
...
def _public_stand(row: sqlite3.Row) -> dict:
    ...
    return {
        ...
        "integrantes":   _json_list(row["integrantes"]),
        "tipo_projeto":  row["tipo_projeto"] or "",
        "link_projeto":  row["link_projeto"] or "",
        "githubs":       _json_list(row["githubs"]),
        "tipos_arquivo": _json_list(row["tipos_arquivo"]),
    }
...
def create_stand(
    ...
    tipo_projeto: str = "",
    link_projeto: str = "",
    githubs: list[str] | None = None,
    tipos_arquivo: list[str] | None = None,
) -> tuple[dict, str]:
    """
    Cadastra novo stand — feito pelo professor, com os dados do grupo.
    ...
    integrantes = [i for i in (integrantes or []) if i]
    tipos_arquivo = list(dict.fromkeys(t for t in (tipos_arquivo or []) if t in VALID_FILE_TYPES))
    # GitHub individual só faz sentido para Programação e Web Design.
    githubs = [g for g in (githubs or []) if g] if course in GITHUB_COURSES else []
    ...
    try:
        with _write() as c:
            ...
            c.execute(
                "INSERT INTO stands (id, name, group_id, course, code_salt, code_hash, "
                "created_at, integrantes, tipo_projeto, link_projeto, githubs, tipos_arquivo) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    stand_id, name, group_id, course, salt, code_hash, _now(),
                    json.dumps(integrantes), tipo_projeto, link_projeto,
                    json.dumps(githubs), json.dumps(tipos_arquivo),
                ),
            )
            row = _get_stand(c, stand_id)
