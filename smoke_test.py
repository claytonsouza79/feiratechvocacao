...
class FeiraTechSmokeTest(unittest.TestCase):
    ...
    def _create_stand(self, **fields):
        """Cadastra um stand como o professor faria, no painel admin."""
        fields.setdefault("password", os.environ["ADMIN_PASSWORD"])
        fields.setdefault("integrantes", ["Aluno Teste"])
        return self.client.post("/api/stands", json=fields)
    ...
    def test_stand_duplicado_e_qrcode(self):
        first = self._create_stand(**{"name": "Robótica Legal", "course": "programacao"})
        self.assertEqual(first.status_code, 201)
        code = first.get_json()["access_code"]
        stand_id = first.get_json()["stand"]["id"]

        duplicated = self._create_stand(**{"name": "robotica legal!", "course": "programacao"})
    ...
    def test_cadastro_unico_e_passaporte(self):
        ...
        stand_a = self._create_stand(**{
            "name": "Projeto A", "course": "webdesign",
        }).get_json()["stand"]["id"]
        stand_b = self._create_stand(**{
    ...
    def test_admin_exclui_stand_e_dados_ligados(self):
        ...
        stand = self._create_stand(**{
            "name": "Stand Para Excluir", "course": "webdesign",
        }).get_json()["stand"]
        outro = self._create_stand(**{
        ...
        recriado = self._create_stand(**{
            "name": "Stand Para Excluir", "course": "webdesign",
        })
        self.assertEqual(recriado.status_code, 201)

    def test_aluno_marca_cursos_sem_stand(self):
        senha = os.environ["ADMIN_PASSWORD"]
        todos = ["programacao", "webdesign", "audiovisual", "ppt",
                 "informatica", "excel", "rh", "administrativo"]

        # Aluno de Excel/RH sem stand: precisa avisar que não tem stand
        sem_aviso = self.client.post("/api/visitors/profile", json={
            "visitor_key": "aluno_sem_stand_1234567890",
            "profile_type": "aluno",
            "name": "Aluno Sem Aviso",
            "contact": "aluno@teste.com",
            "cursos": ["excel", "rh"],
        })
        self.assertEqual(sem_aviso.status_code, 400)

        ok = self.client.post("/api/visitors/profile", json={
            "visitor_key": "aluno_sem_stand_1234567890",
            "profile_type": "aluno",
            "name": "Aluno Excel Rh",
            "contact": "aluno@teste.com",
            "cursos": ["excel", "rh", "curso-que-nao-existe"],
            "no_stand": True,
        })
        self.assertEqual(ok.status_code, 201)

        # Os oito cursos são aceitos e o curso inválido é ignorado
        todos_ok = self.client.post("/api/visitors/profile", json={
            "visitor_key": "aluno_todos_cursos_1234567890",
            "profile_type": "aluno",
            "name": "Aluno Todos Cursos",
            "contact": "todos@teste.com",
            "cursos": todos,
            "no_stand": True,
        })
        self.assertEqual(todos_ok.status_code, 201)

        csv_txt = self.client.get(
            f"/api/admin/visitors/export?password={senha}"
        ).get_data(as_text=True)
        self.assertIn("Excel; Assistente de RH", csv_txt)
        self.assertNotIn("curso-que-nao-existe", csv_txt)
        for rotulo in ("Informática Básica", "Auxiliar Administrativo"):
            self.assertIn(rotulo, csv_txt)

    def test_cadastro_de_stand_e_do_professor_com_dados_completos(self):
        # Sem senha, ninguém cadastra stand
        sem_senha = self.client.post("/api/stands", json={
            "name": "Sem Senha", "course": "programacao", "integrantes": ["Ana"],
        })
        self.assertEqual(sem_senha.status_code, 401)

        senha_errada = self._create_stand(name="Senha Errada", course="programacao",
                                           password="errada")
        self.assertEqual(senha_errada.status_code, 401)

        # A feira é só para estes 3 cursos — os demais não geram stand
        curso_fora = self._create_stand(name="Curso Fora", course="ppt")
        self.assertEqual(curso_fora.status_code, 400)

        # Sem integrante, não cadastra
        sem_integrante = self._create_stand(
            name="Sem Integrante", course="webdesign", integrantes=[],
        )
        self.assertEqual(sem_integrante.status_code, 400)

        # Programação/Web Design: GitHub é salvo
        prog = self._create_stand(
            name="App Financeiro",
            course="programacao",
            integrantes=["Ana Souza", "Bruno Lima"],
            tipo_projeto="Aplicativo mobile",
            link_projeto="https://appfinanceiro.exemplo.com",
            githubs=["https://github.com/ana", "https://github.com/bruno"],
            tipos_arquivo=["executavel", "documento", "curso-invalido"],
        ).get_json()["stand"]
        self.assertEqual(prog["integrantes"], ["Ana Souza", "Bruno Lima"])
        self.assertEqual(prog["tipo_projeto"], "Aplicativo mobile")
        self.assertEqual(
            prog["githubs"], ["https://github.com/ana", "https://github.com/bruno"]
        )
        self.assertEqual(prog["tipos_arquivo"], ["executavel", "documento"])

        # Produção Audiovisual: GitHub é ignorado mesmo se enviado
        audio = self._create_stand(
            name="Documentario Vocacao",
            course="audiovisual",
            integrantes=["Carla Dias"],
            githubs=["https://github.com/carla"],
            tipos_arquivo=["videos", "podcast"],
        ).get_json()["stand"]
        self.assertEqual(audio["githubs"], [])
        self.assertEqual(audio["tipos_arquivo"], ["videos", "podcast"])
    ...
    def test_relatorio_consolidado(self):
        ...
        vazio_id = self._create_stand(**{
            "name": "Projeto Sem Visita", "course": "programacao",
        }).get_json()["stand"]["id"]
    ...
    def test_aluno_com_multiplos_cursos_e_stands(self):
        ...
        stand_web = self._create_stand(**{
            "name": "Grupo Web", "course": "webdesign",
        }).get_json()["stand"]["id"]
        stand_prog = self._create_stand(**{
            "name": "Grupo Programação", "course": "programacao",
        }).get_json()["stand"]["id"]
        stand_alheio = self._create_stand(**{
    ...
    def test_nome_de_stand_com_html_e_recusado(self):
        ...
        r = self._create_stand(**{
            "name": "<img src=x onerror=alert(1)>", "course": "webdesign",
        })
        self.assertEqual(r.status_code, 400)

        ok = self._create_stand(**{"name": "Mãos d'Água & Cia", "course": "webdesign"})
        self.assertEqual(ok.status_code, 201)
    ...
    def test_backup_sqlite_baixa_arquivo_valido(self):
        self._create_stand(**{"name": "Stand Backup", "course": "audiovisual"})
