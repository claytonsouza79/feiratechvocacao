...
(() => {
  ...
  const courseLabels = {
    ...
    informatica:    '🖥️ Informática Básica',
    excel:          '📊 Excel',
    rh:             '🤝 Assistente de RH',
    administrativo: '🗂️ Auxiliar Administrativo',
  ...
  // Só Programação e Web Design pedem GitHub individual no cadastro do stand.
  const githubCourses = new Set(['programacao', 'webdesign']);
  ...
  function initCheckin(onComplete, context = {}) {
    ...
    async function submit() {
      ...
      if (selectedType === 'aluno') {
        ...
        const semStand = $('alunoSemStand').checked;
        if (!selectedStands.size && manualGroup.length < 2 && !semStand) {
          toast('Marque o stand do seu grupo, digite o nome do grupo ou marque "Não tenho stand na feira".');
          $('alunoGrupoManual').focus();
          return;
        }
        payload.own_stand_ids = [...selectedStands];
        payload.group_name = manualGroup || null;
        payload.no_stand = semStand;
  ...
  function initAdmin() {
    ...
    $('adminLogout').onclick = () => {
      sessionStorage.removeItem('feira-tech-admin');
      $('adminDashboardArea').hidden = true;
      $('adminLogout').hidden = true;
      $('adminLoginArea').hidden = false;
      $('adminPassword').value = '';
      $('adminLoginForm').scrollIntoView({ behavior: 'smooth', block: 'start' });
    };
  ...
  /** Liga o formulário do professor para cadastrar um novo stand. */
  function setupNewStandForm(password) {
    const selectedFileTypes = new Set();

    $('newStandCourse').onchange = () => {
      $('newStandGithubWrap').hidden = !githubCourses.has($('newStandCourse').value);
    };
    $('newStandCourse').onchange();

    $('newStandFileTypes').onclick = (e) => {
      const pill = e.target.closest('.course-pill');
      if (!pill) return;
      const tipo = pill.dataset.filetype;
      if (selectedFileTypes.has(tipo)) {
        selectedFileTypes.delete(tipo);
        pill.classList.remove('selected');
      } else {
        selectedFileTypes.add(tipo);
        pill.classList.add('selected');
      }
    };

    $('newStandForm').onsubmit = async (e) => {
      e.preventDefault();
      const curso = $('newStandCourse').value;
      const integrantes = $('newStandIntegrantes').value
        .split('\n').map((n) => n.trim()).filter(Boolean);

      if (!integrantes.length) {
        toast('Informe ao menos um integrante do grupo.');
        return;
      }

      const payload = {
        password,
        name: $('newStandName').value.trim(),
        course: curso,
        integrantes,
        tipo_projeto: $('newStandTipoProjeto').value.trim(),
        link_projeto: $('newStandLinkProjeto').value.trim(),
        tipos_arquivo: [...selectedFileTypes],
        githubs: githubCourses.has(curso)
          ? $('newStandGithubs').value.split('\n').map((g) => g.trim()).filter(Boolean)
          : [],
      };

      const button = $('newStandForm').querySelector('button[type="submit"]');
      button.disabled = true;
      try {
        const result = await post('/api/stands', payload);
        $('newStandCode').textContent = result.access_code;
        $('newStandNameReveal').textContent = `Stand: ${result.stand.name}`;
        $('newStandReveal').hidden = false;
        $('newStandForm').reset();
        selectedFileTypes.clear();
        $('newStandFileTypes').querySelectorAll('.course-pill.selected')
